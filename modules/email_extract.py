...
SKIP_TLD = {"png", "jpg", "jpeg", "gif", "svg", "webp", "css", "js", "woff", "woff2", "ttf", "eot"}
SKIP_LOCAL = {"example.com", "domain.com", "yourdomain.com", "email.com", "gmail"}


class EmailExtractTab(Gtk.Box):
    def __init__(self):
        super().__init__(orientation=Gtk.Orientation.VERTICAL, spacing=12)
        self.set_border_width(24)

        title = Gtk.Label(label="E-Posta Toplayici")
        title.get_style_context().add_class("title-label")
        title.set_xalign(0)
        self.pack_start(title, False, False, 0)

        desc = Gtk.Label(label="Sayfa iceriginden e-posta adreslerini cikarir ve tekrarlari temizler. Bakim/eposta adresleri ve sayfa tabanli ornek adresler ayiklanir.")
    ...
    def scan(self, url):
        try:
            ...
            GLib.idle_add(self.log, f"[*] Bulunan: {len(sorted_emails)} benzersiz adres\n")
            GLib.idle_add(self.log, "=== E-POSTA ADRESLERI ===")
            if sorted_emails:
                for e in sorted_emails:
                    GLib.idle_add(self.log, f"  {e}")
            else:
                GLib.idle_add(self.log, "  [-] E-posta bulunamadi")
            GLib.idle_add(self.log, "")
            GLib.idle_add(self.log, "[!] Bulunan adresler yalnizca sirketinizin/izin verilen hedeflerin testidir. Spam/kotu amac icin kullanilamaz.")
