    ...
    def scan(self, url):
        ...
        try:
            ...
            if vulnerable:
                ...
                self.log("")
                self.log("  Bu acik bir saldirganin SU ISLEMLERI yapmasina izin verebilir:")
                self.log("  1. Veritabanindaki TUM verileri okumak")
                self.log("     - kullanici adlari, sifre hashleri, e-posta, adres, telefon")
                self.log("     - oduemeniz / kredi karti bilgileri (varsa)")
                self.log("  2. Giris/oturum dogrulamasini atlatmak")
                self.log("     - admin paneline sifresiz girmek (OR 1=1 teknigi)")
                self.log("  3. Verileri degistirmek veya silmek")
                self.log("     - kayitlari bozmak, veritabanini dusurmek (DoS)")
                self.log("  4. Sunucuda uzak komut calistirmak (en tehlikelisi)")
                self.log("     - MySQL INTO OUTFILE, MSSQL xp_cmdshell yoluyla")
                self.log("  5. Sonuc: SITE TAMAMEN ELE GECIRILIR")
                self.log("")
                self.log("  Nasil dogrulanir / istismar edilir:")
                self.log("  1. sqlmap -u \"" + url + "\" --batch")
                self.log("  2. Manuel: ' UNION SELECT 1,2,3,4...-- -")
                self.log("  3. Tablolari cikarmak icin:")
                self.log("     ' UNION SELECT table_name FROM information_schema.tables-- -")
                self.log("")
                self.log("  Korunma: Parametreli sorgular (prepared statements) kullanin,")
                self.log("           kullanici girdisini asla sorguya dogrudan katmayin.")
            elif diff_found:
                self.log("  [?] Boolean-based zafiyet olabilir (manuel kontrol gerekli)")
                self.log("  Bulunursa saldirgan: veritabani adini, tablolari ve verileri")
                self.log("  karakter karakter cikarabilir (time-based gibi).")
